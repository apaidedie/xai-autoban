...
func (h *Handler) Registration() pluginapi.ManagementRegistrationResponse {
	return pluginapi.ManagementRegistrationResponse{
		Routes: []pluginapi.ManagementRoute{
			{Method: http.MethodGet, Path: ("/plugins/" + h.Name) + "/bans", Description: "List xAI credentials excluded by xai-autoban."},
			{Method: http.MethodGet, Path: ("/plugins/" + h.Name) + "/audit", Description: "List recent autoban audit events."},
			{Method: http.MethodGet, Path: ("/plugins/" + h.Name) + "/settings", Description: "Get effective runtime settings."},
			{Method: http.MethodPut, Path: ("/plugins/" + h.Name) + "/settings", Description: "Update runtime settings (probe actions, intervals, etc.)."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/settings", Description: "Update runtime settings (probe actions, intervals, etc.)."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/unban", Description: "Release one xAI credential. Body: {\"auth_id\":\"...\"}."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/unban-all", Description: "Release all credentials held by xai-autoban."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/import", Description: "Restore a previously exported ban snapshot or full backup JSON."},
			{Method: http.MethodGet, Path: ("/plugins/" + h.Name) + "/backup", Description: "Export bans + settings backup JSON (safe, no secrets)."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/probe", Description: "Run credential probe. Default async; body {wait:true} for sync."},
			{Method: http.MethodGet, Path: ("/plugins/" + h.Name) + "/probe/status", Description: "Probe job progress (done/total)."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/apply-action", Description: "Manually apply ban|disable|delete|reenable|reauth. Body: {\"auth_id\",\"action\",\"force?\"}."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/reauth", Description: "Refresh access_token via refresh_token. Body: {\"auth_id\"}."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/bans-recheck-429", Description: "Probe currently isolated 429 credentials; unban if recovered, else refresh ban window."},
			{Method: http.MethodPost, Path: ("/plugins/" + h.Name) + "/recheck-selected", Description: "Concurrently probe selected credentials (includes disabled). Body: {\"auth_ids\":[],\"reenable_on_ok?\":true}."},
		},
