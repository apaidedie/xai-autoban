...
static int call_host_api(char* method, uint8_t* request, size_t request_len, cliproxy_buffer* response) {
...
func invokeHost(method string, request []byte) ([]byte, error) {
	...
	rc := C.call_host_api(cMethod, reqPtr, C.size_t(len(request)), &buf)
	if buf.ptr != nil {
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if err != nil {
		writeCResponse(response, errorEnvelope("plugin_error", err.Error()))
		return 1
	}
