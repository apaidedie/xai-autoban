...
// HTTPDoer performs one Management API HTTP call.
// Tests inject a stub; production uses direct (no-proxy) net/http.
type HTTPDoer func(req pluginapi.HTTPRequest, timeoutSec int) (pluginapi.HTTPResponse, error)
...
func newManagementDisabler(cfg config.PluginConfig, host host.Client) *managementDisabler {
	return &managementDisabler{cfg: cfg, host: host}
}

func (m *managementDisabler) updateConfig(cfg config.PluginConfig) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.cfg = cfg
}
...
func (m *managementDisabler) baseURL(cfg config.PluginConfig) string {
...
func (m *managementDisabler) patchAuthStatus(do HTTPDoer, cfg config.PluginConfig, key, name string, disabled bool) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return &HTTPError{StatusCode: resp.StatusCode, Body: strings.TrimSpace(string(resp.Body))}
	}
	return nil
}
...
func (m *managementDisabler) patchAuthFields(do HTTPDoer, cfg config.PluginConfig, key, name string, fields map[string]any) error {
...
// deleteAuthFile removes a credential via Management DELETE /auth-files.
// Tries name candidates (id, id.json, listed name) similar to status patch.
func (m *managementDisabler) deleteAuthFile(authID, authIndex string) error {
	return m.deleteAuthFileWithKey(authID, authIndex, m.resolveKey())
}

func (m *managementDisabler) deleteAuthFileWithKey(authID, authIndex, key string) error {
	m.mu.Lock()
	cfg := m.cfg
	blocked := m.blockedUntil
	do := m.httpDo
	...
	var last error
	for _, name := range candidates {
		if err := m.deleteAuthByName(do, cfg, key, name); err == nil {
			m.mu.Lock()
			m.lastError = ""
			m.mu.Unlock()
			return nil
	...
	if last == nil {
		last = fmt.Errorf("delete auth-file failed")
	}
	m.mu.Lock()
	m.lastError = last.Error()
	if IsAuthError(last) {
		sec := cfg.ManagementAuthFailureCooldownSeconds
		if sec <= 0 {
			sec = DefaultMgmtAuthCooldownSec
	...
	m.mu.Unlock()
	return AnnotateError(last)
}

func (m *managementDisabler) deleteAuthByName(do HTTPDoer, cfg config.PluginConfig, key, name string) error {
	...
	if err != nil {
		return fmt.Errorf("management delete failed: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return &HTTPError{StatusCode: resp.StatusCode, Body: strings.TrimSpace(string(resp.Body))}
	}
...
func (m *managementDisabler) findAuthFile(do HTTPDoer, cfg config.PluginConfig, key, authID, authIndex string) (managementAuthFile, bool, error) {
	timeout := cfg.ManagementTimeoutSeconds
	if timeout <= 0 {
		timeout = DefaultMgmtTimeoutSec
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return managementAuthFile{}, false, &HTTPError{StatusCode: resp.StatusCode, Body: strings.TrimSpace(string(resp.Body))}
	}
	var payload struct {
...
// hostHTTPDoer adapts host.Client.HTTPDo for tests that inject stubHost.httpFn.
func HostHTTPDoer(host host.Client) HTTPDoer {
