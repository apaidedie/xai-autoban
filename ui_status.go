...
.chip .l{color:var(--muted);font-size:11px;font-weight:800;letter-spacing:.06em}
...
<div class="shell">
  ...
  <section class="panel">
    <div class="phd"><h2>当前巡检配置</h2><div class="hint">点击右上角「编辑配置」可改巡检后操作</div></div>
    <div class="cfg-summary">
      ...
      <div class="chip"><div class="l">自动执行</div><div class="v" id="sumAutoExec">-</div></div>
      <div class="chip"><div class="l">问题账号策略</div><div class="v" id="sumProbeAction">-</div></div>
      <div class="chip"><div class="l">成功策略</div><div class="v" id="sumOnSuccess">-</div></div>
  ...
  <section class="panel">
    <div class="phd"><h2>巡检历史</h2><div class="hint">内存记录，重启后清空</div></div>
  ...
  <section class="panel">
    ...
    <div class="row">
      <input id="search" type="search" placeholder="搜索 Auth ID / 原因 / 动作" autocomplete="off">
  ...
  <section class="panel">
    <div class="phd"><h2>隔离账本</h2><div class="hint" id="resultCount">0 条</div></div>
  ...
  <p class="foot">
    对齐 CPA-Manager-Plus Codex 巡检：只输出结果 / 自动执行 + 问题账号策略。
    <b>ban</b> 仅插件内存隔离（调度跳过），不会把 CPA 凭证开关打成停用；若要列表显示停用请选 <b>disable</b>。
    运行时配置立即生效；重启后以 config.yaml 为准。
  </p>
</div>
...
function setAuthUI(){
  ...
  b.textContent=ok?'已授权：可解禁 / 巡检 / 编辑配置。':'只读模式：请先保存管理密钥再执行写操作。';
...
function renderSettingsSummary(s){
  ...
  $('sumAutoExec').textContent=auto?'自动执行':'只输出结果';
...
async function unbanOne(id){ if(!confirm('确认解禁？\n'+id)) return; try{ setMessage('正在解禁…'); await apiMgmt('POST','/unban',{auth_id:id}); state.selected.delete(id); setMessage('已解禁'); await loadData(true);}catch(e){setMessage(e.message,true)} }
