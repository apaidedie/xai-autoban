...
func StorageKey(email, authID string) string {
...
func (s *State) Set(authID string, entry Entry) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *State) ForceSet(authID string, entry Entry) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.setLocked(authID, entry, true)
}

func (s *State) ensureMapsLocked() {
	if s.bans == nil {
		s.bans = make(map[string]Entry)
	}
	if s.emailIndex == nil {
...
func (s *State) setLocked(authID string, entry Entry, force bool) {
	s.ensureMapsLocked()
	...
	key := StorageKey(entry.Email, authID)
	if key == "" {
		return
	}
...
func (s *State) indexEntryLocked(key string, entry Entry, related map[string]struct{}) {
	s.ensureMapsLocked()
	if entry.Email != "" {
...
func (s *State) unindexLocked(key string, entry Entry) {
...
func (s *State) Active(authID string, now time.Time) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.activeLocked(authID, now)
}

func (s *State) activeLocked(authID string, now time.Time) bool {
...
func (s *State) lookupLocked(idOrEmail string) (key string, entry Entry, ok bool) {
...
func (s *State) IsBannedCandidateID(id string, now time.Time) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *State) IsBannedCandidate(id, email string, now time.Time) bool {
...
func (s *State) Clear(authID string) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	key, entry, ok := s.lookupLocked(authID)
	if !ok {
...
func (s *State) ClearStatus(status int) int {
	s.mu.Lock()
	defer s.mu.Unlock()
	removed := 0
...
func (s *State) ClearMany(authIDs []string) int {
	s.mu.Lock()
	defer s.mu.Unlock()
	removed := 0
...
func (s *State) Snapshot(now time.Time) map[string]Entry {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *State) ReplaceAll(entries map[string]Entry) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.bans = make(map[string]Entry, len(entries))
	s.emailIndex = make(map[string]string)
	s.authIndex = make(map[string]string)
