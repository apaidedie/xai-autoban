...
### UX
- 巡检结果按状态码汇总：401/402/403/429/4xx/5xx/other
- 展示已执行动作（隔离/禁用/删除）与合计；完成面板与巡检记录更详细

## 1.2.0 - 2026-07-19

### Ops
- 运维台可配 **402/403/429 隔离时长（秒）**
- **禁用池**概览：总数 · 上次巡检成/败/未知 · 均龄
- 保存配置 **hints**：「仅已禁用」与 402/429 隔离自洽说明
- 巡检卡：候选=已禁用/全部；跳过拆成流量宽限 + 近期成功
- 批量 **清除禁用上 403 残留隔离**；**导出禁用池** JSON
- Management 密钥/错误 **顶栏常亮警告**
- 禁用池专用并发/QPS（`probe_disabled_*`）
- Reconcile 节流（status 5 分钟；启动/保存强制）
- state 文件紧凑 JSON + 过期 bans 不落盘

## 1.1.9 - 2026-07-19

### Behavior
- **禁用与隔离互斥**：已禁用凭证只保留「禁用」状态；禁止再写隔离账本；启动/运维台刷新时自动清掉「禁用+隔离」残留
- 对已禁用号触发隔离动作 → 跳过并清理账本（直至巡检成功「启用」或永久禁用）

## 1.1.8 - 2026-07-19

### UX
- 概览计数说明：健康/隔离/禁用**可重叠**；展示「兼禁用 / 仅禁用」副文案，避免误以为三者相加应等于全部
- 用语说明补充：全部 = 健康 + 隔离 + 仅禁用；401–429 之和 = 隔离

## 1.1.7 - 2026-07-18

### UX / 用语
- 统一运维台用语：**隔离 / 释放 / 禁用 / 启用 / 巡检 / 复检 / 状态码动作 / 到期自动释放**
- 「探测」统一为「巡检」；「取消隔离」统一为「释放」；「兼隔离」→「仍隔离」
- 配置抽屉：失败策略标明「兜底」；状态码区标题明确真实失败/巡检/复检共用
- README 对齐 1.1.6+ 行为与用语表

## 1.1.6 - 2026-07-18

### Behavior
- **禁用不再写入隔离账本**：`action_on_403=disable` 只关 CPA 开关，并清除该号已有隔离；401–429 卡片只统计「隔离」动作留下的状态码
- 成功策略「启用 / 释放并启用」负责巡检恢复后打开开关
